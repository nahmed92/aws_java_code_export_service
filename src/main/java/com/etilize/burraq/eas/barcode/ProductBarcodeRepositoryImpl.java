...
/**
 * It is implementation of {@link ProductBarcodeCustomRepository}
 *
 * @author Umar Zubair
 * @since 1.0
 */
public class ProductBarcodeRepositoryImpl implements ProductBarcodeCustomRepository {
     ...
     */
    @Autowired
    public ProductBarcodeRepositoryImpl(final AmazonDynamoDB amazonDynamoDB) {
        Assert.notNull(amazonDynamoDB, "amazonDynamoDB is required.");
        final DynamoDB db = new DynamoDB(amazonDynamoDB);
        table = db.getTable(ProductBarcode.TABLE_NAME);
    }

    private void insert(final ProductBarcode barcode) {
    ...
    private void update(final ProductBarcode barcode) {
    ...
    @Override
    public void link(final ProductBarcode barcode) {
        try {
            update(barcode);
    ...
    @Override
    public void unlink(final ProductBarcode barcode) {
        final NameMap nameMap = new NameMap();
        final ValueMap valueMap = new ValueMap() //
                .withLong(COLON_LAST_UPDATE_DATE, barcode.getLastUpdateDate().getTime());
