...
/**
 * It is an implementation of {@link BasicMediaSpecificationCustomRepository}
 *
 * @author Umar Zubair
 * @since 1.0
 */
public class BasicMediaSpecificationRepositoryImpl
        implements BasicMediaSpecificationCustomRepository {
     ...
     */
    @Autowired
    public BasicMediaSpecificationRepositoryImpl(final AmazonDynamoDB amazonDynamoDB) {
        Assert.notNull(amazonDynamoDB, "amazonDynamoDB is required.");
        final DynamoDB db = new DynamoDB(amazonDynamoDB);
        table = db.getTable(BasicMediaSpecification.TABLE_NAME);
    }
