...
/**
 * It defines implementation customization for {@link ProductAccessorySpecificationRepository}
 *
 * @author Kaiser Raja
 * @since 1.0
 */
public class ProductAccessorySpecificationRepositoryImpl
        implements ProductAccessorySpecificationCustomRepository {
     ...
     */
    @Autowired
    public ProductAccessorySpecificationRepositoryImpl(final AmazonDynamoDB amazonDynamoDB) {
        Assert.notNull(amazonDynamoDB, "amazonDynamoDB is required.");
        final DynamoDB db = new DynamoDB(amazonDynamoDB);
        table = db.getTable(ProductAccessorySpecification.TABLE_NAME);
    }
    ...
    public Optional<ProductSpecification> findOne(final String id) {
        ...
        ProductAccessorySpecification specs = null;
        if (item != null) {
            specs = new ProductAccessorySpecification();
            specs.setId(id);
