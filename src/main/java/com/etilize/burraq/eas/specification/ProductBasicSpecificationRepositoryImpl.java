...
/**
 * It defines implementation customization for {@link ProductBasicSpecificationRepository}
 *
 * @author Umar Zubair
 * @since 1.0
 */
public class ProductBasicSpecificationRepositoryImpl
        implements ProductBasicSpecificationCustomRepository {
     ...
     */
    @Autowired
    public ProductBasicSpecificationRepositoryImpl(final AmazonDynamoDB amazonDynamoDB) {
        Assert.notNull(amazonDynamoDB, "amazonDynamoDB is required.");
        final DynamoDB db = new DynamoDB(amazonDynamoDB);
        table = db.getTable(ProductBasicSpecification.TABLE_NAME);
    }
    ...
    public Optional<ProductSpecification> findOne(final String id) {
        ...
        ProductBasicSpecification specs = null;
        if (item != null) {
            specs = new ProductBasicSpecification();
            specs.setId(id);
