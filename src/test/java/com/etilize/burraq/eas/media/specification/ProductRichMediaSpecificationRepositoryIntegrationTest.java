...
/**
 * This class implements repository test cases for {@link ProductRichMediaSpecificationRepository}.
 *
 * @author Umar Zubair
 * @since 1.0
 */
@UsingDataSet(locations = "/datasets/rich_media_specifications/rich_media_specifications.bson", loadStrategy = LoadStrategyEnum.CLEAN_INSERT)
@CustomComparisonStrategy(comparisonStrategy = DynamoFlexibleComparisonStrategy.class)
public class ProductRichMediaSpecificationRepositoryIntegrationTest
        extends AbstractIntegrationTest {

    @Autowired
    private ProductRichMediaSpecificationRepository repository;

    @Test
    public void shouldFindAllRichMediaSpecifications() {
        final List<ProductRichMediaSpecification> specifications = Lists.newArrayList(
                repository.findAll());
    ...
    @Test
    public void shouldFindRichMediaSpecificationById() {
        final Optional<ProductRichMediaSpecification> specification = repository.findById(
                "product123-en");
    ...
    public void shouldCreateNewRichMediaSpecification() {
        ...
        final ProductRichMediaSpecification specs = new ProductRichMediaSpecification();
    ...
    public void shouldUpdateRichMediaSpecification() {
        ...
        value.setWidth(160);
        value.setTags(Sets.newHashSet("Prod Specs"));
        final ProductRichMediaSpecification specs = repository.findById(
                "product123-en_US").get();
        specs.getAttributes().put("maxId", value);
        repository.save(specs);
    }

    @ShouldMatchDataSet(location = "/datasets/rich_media_specifications/rich_media_specifications_after_delete.bson")
    @Test
    public void shouldDeleteRichMediaSpecificationById() {
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/rich_media_specifications/rich_media_specifications_after_delete_attribute_one_record.bson")
    @IgnorePropertyValue(properties = { "lastUpdateDate" })
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/rich_media_specifications/rich_media_specifications.bson")
    @IgnorePropertyValue(properties = { "lastUpdateDate" })
