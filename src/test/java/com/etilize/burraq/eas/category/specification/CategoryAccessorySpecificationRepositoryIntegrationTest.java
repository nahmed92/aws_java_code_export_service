...
import com.etilize.burraq.eas.category.specification.CategoryAccessorySpecification;
import com.etilize.burraq.eas.category.specification.CategoryAccessorySpecificationRepository;
import com.etilize.burraq.eas.category.specification.CategorySpecificationKey;
...
/**
 * This class implements repository test cases for {@link CategoryAccessorySpecificationRepository}.
 *
 * @author Kaiser Raja
 * @since 1.0
 */
@UsingDataSet(locations = "/datasets/accessory_category_structures/accessory_category_structures.bson", loadStrategy = LoadStrategyEnum.CLEAN_INSERT)
@CustomComparisonStrategy(comparisonStrategy = DynamoFlexibleComparisonStrategy.class)
public class CategoryAccessorySpecificationRepositoryIntegrationTest
        extends AbstractIntegrationTest {

    @Autowired
    private CategoryAccessorySpecificationRepository repository;

    @Test
    public void shouldFindAccessoryCategoryStructuresByMultipleIds() {
        final CategorySpecificationKey key1 = new CategorySpecificationKey();
        key1.setCategoryId("categoryId123");
        key1.setLocaleId("en_CA");
        final CategorySpecificationKey key2 = new CategorySpecificationKey();
        key2.setCategoryId("categoryId123");
        key2.setLocaleId("en_US");
        final Iterator<CategoryAccessorySpecification> categories = repository.findAllById(
                Lists.newArrayList(key1, key2)).iterator();
        assertThat(categories.hasNext(), is(true));
        CategoryAccessorySpecification category = categories.next();
    ...
    @Test
    public void shouldFindAccessoryCategoryStructuresById() {
        final CategorySpecificationKey key = new CategorySpecificationKey();
        key.setCategoryId("categoryId123");
        key.setLocaleId("en_CA");
        final Optional<CategoryAccessorySpecification> category = repository.findById(key);
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/accessory_category_structures/accessory_category_structures_after_create.bson")
    public void shouldCreateNewAccessoryCategoryStructure() {
        final CategoryAccessorySpecification category = new CategoryAccessorySpecification();
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/accessory_category_structures/accessory_category_structures_after_update.bson")
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/accessory_category_structures/accessory_category_structures_after_delete.bson")
    public void shouldDeleteAccessoryCategoryStructureById() {
        final CategorySpecificationKey key = new CategorySpecificationKey();
        key.setCategoryId("categoryId123");
        key.setLocaleId("en_CA");
