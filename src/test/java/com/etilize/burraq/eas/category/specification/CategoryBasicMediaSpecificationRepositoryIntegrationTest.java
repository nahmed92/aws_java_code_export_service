...
import com.etilize.burraq.eas.category.specification.CategoryBasicMediaSpecification;
...
/**
 * This class implements repository test cases for {@link CategoryBasicMediaSpecificationRepository}.
 *
 * @author Umar Zubair
 * @since 1.0
 */
@UsingDataSet(locations = "/datasets/basic_media_category_structures/basic_media_category_structures.bson", loadStrategy = LoadStrategyEnum.CLEAN_INSERT)
@CustomComparisonStrategy(comparisonStrategy = DynamoFlexibleComparisonStrategy.class)
public class CategoryBasicMediaSpecificationRepositoryIntegrationTest
        extends AbstractIntegrationTest {

    @Autowired
    private CategoryBasicMediaSpecificationRepository repository;

    @Test
    public void shouldFindBasicMediaCategoryStructuresByMultipleIds() {
        ...
        final Iterator<CategoryBasicMediaSpecification> categories = repository.findAllById(
                Lists.newArrayList(key1, key2)).iterator();
        assertThat(categories.hasNext(), is(true));
        CategoryBasicMediaSpecification category = categories.next();
    ...
    @Test
    public void shouldFindBasicMediaCategoryStructuresById() {
        final CategorySpecificationKey key = new CategorySpecificationKey();
        key.setCategoryId("categoryId123");
        key.setLocaleId("en_CA");
        final Optional<CategoryBasicMediaSpecification> category = repository.findById(key);
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/basic_media_category_structures/basic_media_category_structures_after_create.bson")
    public void shouldCreateNewBasicMediaCategoryStructure() {
        final CategoryBasicMediaSpecification category = new CategoryBasicMediaSpecification();
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/basic_media_category_structures/basic_media_category_structures_after_update.bson")
    ...
    @Test
    @ShouldMatchDataSet(location = "/datasets/basic_media_category_structures/basic_media_category_structures_after_delete.bson")
    public void shouldDeleteBasicMediaCategoryStructureById() {
        final CategorySpecificationKey key = new CategorySpecificationKey();
        key.setCategoryId("categoryId123");
        key.setLocaleId("en_CA");
